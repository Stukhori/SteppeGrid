...
def flatten(result,registry):
    ...
    if dispatch.is_file():
        with dispatch.open(encoding="utf-8") as h:
            for row in csv.DictReader(h): wind+=float(row["wind_generation_kwh"]); pv+=float(row["pv_generation_kwh"])
    return {"site_id":result["site_id"],"site":site.name,"region":site.region,"cohort":"primary_proxy" if result["site_id"] in PROXY_IDS else "contextual","demand_basis":result["demand_source_type"],"demand_id":result.get("demand_id"),"demand_sha256":result["demand_sha256"],"weather_sha256":result["weather_sha256"],"scenario_hash":result["scenario_input_hash"],"target":result["reliability_target"],"annual_demand_kwh":annual,"wind_key":d.get("wind_key"),"wind_count":d.get("wind_count"),"wind_capacity_kw":d.get("wind_capacity_kw"),"pv_key":d.get("pv_key"),"pv_count":d.get("pv_count"),"pv_dc_capacity_kw":d.get("pv_dc_capacity_kw"),"pv_ac_capacity_kw":d.get("pv_ac_capacity_kw"),"battery_key":d.get("battery_key"),"battery_count":d.get("battery_count"),"battery_usable_capacity_kwh":d.get("battery_usable_capacity_kwh"),"annual_wind_generation_kwh":wind,"annual_pv_generation_kwh":pv,"raw_generation_kwh":raw,"served_energy_kwh":m.get("served_energy_kwh"),"unmet_energy_kwh":m.get("unmet_energy_kwh"),"curtailment_kwh":m.get("curtailment_kwh"),"served_fraction":m.get("served_fraction"),"lpsp":m.get("lpsp"),"loss_of_load_hours":m.get("loss_of_load_hours"),"longest_deficit_hours":m.get("longest_deficit_hours"),"maximum_hourly_deficit_kwh":m.get("maximum_hourly_deficit_kwh"),"capex_usd":e.get("initial_capex_usd"),"npc_usd":e.get("net_present_cost_usd"),"eac_usd":e.get("equivalent_annual_cost_usd"),"cost_per_served_kwh_usd":e.get("cost_per_served_kwh_usd"),"wind_MW_per_GWh":d.get("wind_capacity_kw",0)/1000/gwh,"pv_MWac_per_GWh":d.get("pv_ac_capacity_kw",0)/1000/gwh,"battery_MWh_per_GWh":d.get("battery_usable_capacity_kwh",0)/1000/gwh,"NPC_per_annual_kWh_demand":e.get("net_present_cost_usd",0)/annual,"CAPEX_per_annual_kWh_demand":e.get("initial_capex_usd",0)/annual,"EAC_per_annual_kWh_demand":e.get("equivalent_annual_cost_usd",0)/annual,"raw_generation_load_ratio":raw/annual,"curtailment_fraction":m.get("curtailment_kwh",0)/raw if raw else 0,"unmet_fraction":m.get("unmet_energy_kwh",0)/annual,"wind_share_raw_generation":wind/raw if raw else 0,"pv_share_raw_generation":pv/raw if raw else 0,"runtime_seconds":result.get("phase17_runtime_seconds",result.get("elapsed_seconds",0)),"evaluated_portfolios":result.get("evaluated_portfolios",0),"dispatch_simulations":result.get("dispatch_simulations",0),"dispatch_cache_hits":result.get("dispatch_cache_hits",0)}
...
def analyze():
    ...
    norm_keys=("site_id","site","cohort","target","annual_demand_kwh","wind_MW_per_GWh","pv_MWac_per_GWh","battery_MWh_per_GWh","NPC_per_annual_kWh_demand","CAPEX_per_annual_kWh_demand","EAC_per_annual_kWh_demand","cost_per_served_kwh_usd","raw_generation_load_ratio","curtailment_fraction","unmet_fraction","wind_share_raw_generation","pv_share_raw_generation")
    ...
    for s in SITE_IDS:
        ...
        escalation.append({"site_id":s,"site":a["site"],"delta_wind_capacity_percent":pct(a["wind_capacity_kw"],b["wind_capacity_kw"]),"wind_capacity_absolute_increase_kw":b["wind_capacity_kw"]-a["wind_capacity_kw"],"wind_entered_at_99":a["wind_capacity_kw"]==0 and b["wind_capacity_kw"]>0,"delta_pv_capacity_percent":pct(a["pv_ac_capacity_kw"],b["pv_ac_capacity_kw"]),"pv_capacity_absolute_increase_kw":b["pv_ac_capacity_kw"]-a["pv_ac_capacity_kw"],"pv_entered_at_99":a["pv_ac_capacity_kw"]==0 and b["pv_ac_capacity_kw"]>0,"delta_storage_energy_percent":pct(a["battery_usable_capacity_kwh"],b["battery_usable_capacity_kwh"]),"storage_absolute_increase_kwh":b["battery_usable_capacity_kwh"]-a["battery_usable_capacity_kwh"],"delta_CAPEX_percent":pct(a["capex_usd"],b["capex_usd"]),"delta_NPC_percent":pct(a["npc_usd"],b["npc_usd"]),"delta_EAC_percent":pct(a["eac_usd"],b["eac_usd"]),"delta_curtailment_kwh":b["curtailment_kwh"]-a["curtailment_kwh"],"delta_LOLH":b["loss_of_load_hours"]-a["loss_of_load_hours"],"delta_unmet_energy_kwh":b["unmet_energy_kwh"]-a["unmet_energy_kwh"]})
...
def figures(rr,flat,esc):
    ...
    for target,n in ((.95,"03"),(.99,"04")):
        rows=[r for r in flat if r["target"]==target]; chart(f"{n}_normalized_capacity_{int(target*100)}.png",f"{target:.0%} normalized renewable capacity",[r["site"] for r in rows],[("Wind MW/GWh",[r["wind_MW_per_GWh"] for r in rows],"#2878A6"),("PV MW/GWh",[r["pv_MWac_per_GWh"] for r in rows],"#D59A16")],"MW per GWh annual demand")
    chart("05_normalized_npc.png","Normalized net present cost",labels,[("95%",[next(x for x in flat if x["site"]==s and x["target"]==.95)["NPC_per_annual_kWh_demand"] for s in labels],"#1F6B5B"),("99%",[next(x for x in flat if x["site"]==s and x["target"]==.99)["NPC_per_annual_kWh_demand"] for s in labels],"#7359A6")],"NPC / annual kWh demand")
    chart("06_npc_escalation.png","95% to 99% NPC escalation",[r["site"] for r in esc],[("NPC increase",[r["delta_NPC_percent"] for r in esc],"#B06F2E")],"Increase (%)"); chart("07_curtailment.png","Curtailment fraction",labels,[("95%",[next(x for x in flat if x["site"]==s and x["target"]==.95)["curtailment_fraction"] for s in labels],"#1F6B5B"),("99%",[next(x for x in flat if x["site"]==s and x["target"]==.99)["curtailment_fraction"] for s in labels],"#B06F2E")],"Fraction of raw generation"); chart("08_storage_intensity.png","Storage intensity",labels,[("95%",[next(x for x in flat if x["site"]==s and x["target"]==.95)["battery_MWh_per_GWh"] for s in labels],"#7359A6"),("99%",[next(x for x in flat if x["site"]==s and x["target"]==.99)["battery_MWh_per_GWh"] for s in labels],"#A28AD0")],"Battery MWh per GWh annual demand")
